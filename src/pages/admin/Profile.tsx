...
const AdminProfile = () => {
  ...
  useEffect(() => {
    const getProfile = async () => {
      if (user?.id) {
        setLoading(true);
        const fetchedProfiles = await fetchProfiles('admin');
        const adminProfile = fetchedProfiles.find(p => p.id === user.id);
        setProfile(adminProfile || null);
        setLoading(false);
      }
    };
    getProfile();
  ...
  if (!profile) {
    return (
      <Card>
        ...
        <CardContent>
          <p>Unable to load admin profile. Please contact support.</p>
        </CardContent>
      </Card>
    );
  }
  ...
  return (
    <Card>
      <CardHeader>
        <ProfileHeader name={`${profile.first_name} ${profile.last_name || ''}`.trim()} subtitle="Admin Profile Details" />
      </CardHeader>
      <CardContent>
